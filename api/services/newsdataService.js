...
function handleError(error, isGlobal, countryCode, params) {
  ...
  if (error.response) {
    ...
    if (status === 429) {
      ...
      message = 'Invalid or unauthorized API key. Please check your NEWSDATA_API_KEY.'
    } else if (status === 422) {
      return { 
        articles: [], 
        totalCount: 0, 
    ...
    } else if (apiData && apiData.results && apiData.results.message) {
      message = apiData.results.message
    }
  } else {
  ...
  if (!params.apikey || params.apikey === 'your_newsdata_api_key' || params.apikey.includes('pub_your')) {
    return {
      articles: [{ 
        ...
        description: "Your NEWSDATA_API_KEY is missing or invalid in environment variables.", 
        country_code: "US", 
        category: "politics" 
      }],
      totalCount: 1, 
